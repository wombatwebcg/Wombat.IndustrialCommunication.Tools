...
public sealed class ComponentCatalogService
{
    public IReadOnlyList<CommunicationComponentDefinition> GetComponents()
    {
        ...
        [
            ...
            new CommunicationComponentDefinition(CommunicationComponentKind.ModbusRtuBluetoothClient, "蓝牙透传 Modbus RTU 客户端", "客户端", "适用于 RFCOMM/SPP 风格蓝牙透传设备调试，连接成功后即可复用统一地址工作区执行读写。", "支持蓝牙设备发现、服务 UUID 选择、自动重连和统一地址读写。", true, false, ["蓝牙", "透传", "Modbus RTU", "统一地址", "读写"], ["1;40001", "1;3;0", "1;40003"], ["优先选择已配对设备。", "默认服务 UUID 为 00001101-0000-1000-8000-00805F9B34FB。", "若服务发现失败，可手工填写 ServiceId 后继续连接。"]),
            new CommunicationComponentDefinition(CommunicationComponentKind.ModbusRtuBluetoothServer, "蓝牙透传 Modbus RTU 服务端", "服务端", "通过本机蓝牙适配器发布 BLE 服务并模拟 Modbus RTU 从站，适合与 BLE 主站联调。", "支持本机 BLE Service/Characteristic 配置、蓝牙监听与 DataStore 内存编辑。", false, true, ["蓝牙", "本机服务", "DataStore", "服务端"], ["CoilDiscretes @ 0", "HoldingRegisters @ 0", "InputRegisters @ 10"], ["该组件使用本机蓝牙，不扫描远端设备。", "可按需修改本机 BLE Service/Characteristic UUID。", "监听后请在内存编辑器读写 DataStore。"]),
